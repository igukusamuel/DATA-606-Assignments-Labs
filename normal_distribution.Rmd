...
```{r load-library mosaic, include =FALSE}
library(mosaic)
```
...
1.  Make a histogram of men's heights and a histogram of women's heights.  How 
    would you compare the various aspects of the two distributions?
        ...
        Both distributions are unimodal and bell shaped.
    
```{r}
mean_height_men <- mean(~hgt, data = mdims)
sd_height_men <- sd(~hgt, data = mdims)


histogram(~hgt, data = mdims, fit = "normal", nint = 8, xlab = "Mens Heights")
histogram(~hgt, data = fdims, fit = "normal", nint = 8, xlab = "Womens Heights")

histogram(~hgt | sex, data = bdims, fit = "normal", nint = 8)
...
2.  Based on the this plot, does it appear that the data follow a nearly normal 
    distribution? 
        ...
        Yes it does. It is both unimodal and symmetrical along the mean.
...
3.  Make a normal probability plot of `sim_norm`.  Do all of the points fall on 
    ...
    data?
        ...
        Not all points fall on the line, but they are all close enough with a few outliers on both ends. This is very similar to the real data.
...
4.  Does the normal probability plot for `fdims$hgt` look similar to the plots 
    ...
    female heights are nearly normal? 
        ...
        The two plots are very similar and its evident that female heights are nearly normally distributed.
...
```{r} # simulated weights data
wgt_norm <- rnorm(fdims$wgt, mean = mean(fdims$wgt), sd = sd(fdims$wgt))
qqnormsim(wgt_norm)

```

        In conclusion, female weights appear to be normally distributed as well.
...
What is the proportion of female heights below 178cm?

```{r}
pnorm(q = 178, mean = fhgtmean, sd = fhgtsd)
sum(fdims$hgt < 178) / length(fdims$hgt)
```

What is the proportion of female weights above 75?

```{r}
1 - pnorm(q = 75, mean = mean(fdims$wgt), sd = sd(fdims$wgt))
sum(fdims$wgt > 75) / length(fdims$wgt)
```

        The heights data is nore closely matched. It is much closer to the normal distribution than the weights data.
...
-   Note that normal probability plots C and D have a slight stepwise pattern.  
    Why do you think this is the case?
        ...
        They are probable discrete measures.
...
hist(fdims$kne.di)
```
        Judging from the plot the kne.di is right skewed.
